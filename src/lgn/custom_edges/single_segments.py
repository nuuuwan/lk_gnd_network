    ...
    for i in range(n -1):
        ...
        for j in range(i+ 1, n):
            ...
            if edge_pair in network.edge_pair_list:
                continue
            distance = shape_utils.compute_distance(network.node_idx[node_i]['centroid'], network.node_idx[node_j]['centroid'])
...
def rebuild_incr(network, n_segments):
    ...
    for i_segment in range(n_segments):
        ...
        log.debug(f'rebuild_incr: { i_segment + 1}/{n_segments} {format_time(average_meet_time)} {network_length:.1f}km {reduction:.1f}min/km {best_edge_pair}') 
...
def rebuild_actual(network):
    network.edge_pair_list = expand('Colombo','Gampaha', 'Kegalle', 'Kandy', "Nuwara Eliya", "Badulla") + expand('Colombo','Kalutara', "Galle", "Matara", "Hambantota")
    return network